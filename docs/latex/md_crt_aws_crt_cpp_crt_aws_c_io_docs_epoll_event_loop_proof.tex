...
Specification / Properties ({\ttfamily preamble.\+h})\+:
\begin{DoxyItemize}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md246}{}\doxysection{Assumptions}\label{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md246}
...
\begin{DoxyItemize}
\item Well-\/behaved client\+: all client threads use the event loop API in a manner that adheres to the specification. A badly-\/behaved client can invalidate the proven properties. For example, a client that reads, writes, or frees a task struct object that is scheduled on the event loop is racy and no longer thread safe. The specification given in {\ttfamily preamble.\+h} forbids this behavior (the ownership of the task struct changes as a result of the schedule function) but we cannot, in general, enforce this behavior since we do not verify client code.
...
read, write
\end{DoxyVerb}
 The contracts are given in the {\ttfamily preamble.\+h} and proof files. The contracts are assumed, not proven. The memory safety of the AWS C Common linked list functions have been proven in CBMC.
...
\end{DoxyItemize}\hypertarget{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md247}{}\doxysection{Simplifications}\label{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md247}
...
\end{DoxyItemize}\hypertarget{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md248}{}\doxysection{Trusted computing base}\label{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md248}
...
\end{DoxyItemize}\hypertarget{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md249}{}\doxysection{References}\label{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md249}
\mbox{[}N1570\mbox{]} ISO/\+IEC. Programming languages – C. International standard 9899\+:201x, 2011\hypertarget{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md250}{}\doxysection{Appendix}\label{md_crt_aws_crt_cpp_crt_aws_c_io_docs_epoll_event_loop_proof_autotoc_md250}
