...
static void s_printHelp()
{
    ...
    fprintf(
        stdout,
        "  update-desired <desired-state-as-JSON> -- updates the desired state of the IoT thing's shadow.  If the "
        "shadow does not exist, it will be created.\n");
    fprintf(
        stdout,
        "  update-reported <reported-state-as-JSON> -- updates the reported state of the IoT thing's shadow.  If the "
        "shadow does not exist, it will be created.\n\n");
}
...
static void s_handleGetShadow(const ApplicationContext &context)
{
    ...
    context.m_shadowClient->GetShadow(
        request,
        [&getWaiter](GetShadowResult &&result)
        {
...
static void s_handleDeleteShadow(const ApplicationContext &context)
{
    ...
    context.m_shadowClient->DeleteShadow(
        request,
        [&deleteWaiter](DeleteShadowResult &&result)
        {
...
static void s_handleUpdateDesiredShadow(const String &params, const ApplicationContext &context)
{
    ...
    context.m_shadowClient->UpdateShadow(
        request,
        [&updateWaiter](UpdateShadowResult &&result)
        {
...
static std::shared_ptr<Aws::Iot::RequestResponse::IStreamingOperation> s_createShadowUpdatedStream(
    const ApplicationContext &context)
{
    ...
    options.WithStreamHandler(
        [](Aws::Iotshadow::ShadowUpdatedEvent &&event)
        {
...
static std::shared_ptr<Aws::Iot::RequestResponse::IStreamingOperation> s_createShadowDeltaUpdatedStream(
    const ApplicationContext &context)
{
    std::promise<bool> subscribedWaiter;
    ...
    options.WithStreamHandler(
        [](Aws::Iotshadow::ShadowDeltaUpdatedEvent &&event)
        {
            Aws::Crt::JsonObject jsonObject;
            event.SerializeToObject(jsonObject);
            Aws::Crt::String json = jsonObject.View().WriteCompact(true);
...
int main(int argc, char *argv[])
{
    ...
    if (!shadowClient)
    {
        printf("Failed to create shadow client with error code %d: %s", LastError(), ErrorDebugString(LastError()));
        return -1;
    }
