...
namespace MqttOperationQueue
{
    ...
    /**
     * Invoked when an operation was removed from the queue and attempted to be sent to the MQTT connection, but it
     * failed.
     */
    ...
    class MqttOperationQueue : public std::enable_shared_from_this<MqttOperationQueue>
    {
      public:
        ...
        static std::shared_ptr<MqttOperationQueue> NewOperationQueue(
            MqttOperationQueueBuilder &options,
         ...
         * @return QueueResult The result of adding the operation to the queue.
         */
        QueueResult Unsubscribe(const char *topicFilter, const Aws::Crt::Mqtt::OnOperationCompleteHandler onOpComplete);
         ...
         * @return QueueResult The result of adding the operation to the queue.
         */
        QueueResult AddQueueOperation(QueueOperation operation);
      ...
      private:
        ...
        /**
         * Helper function: Checks the MQTT connection operation statistics to see if their values are higher than the
         * maximum values set in MqttOperationQueue. If the value is higher than the maximum in MqttOperationQueue, then
         * it returns false so an operation on the queue will not be processed. Called by the ProcessQueue() function.
    ...
    class MqttOperationQueueBuilder
    {
      public:
         ...
         * The default is DROP_BACK, which will drop the newest (but last to be executed) operation at the back of the
         * queue.
         *
         ...
         * sent to the MqttClientConnection but have not been fully processed and responded to from the MQTT
         * server/broker.
         *
        ...
        /**
         * Sets the interval, in milliseconds, that the MqttOperationQueue will wait before checking the queue and
         * (possibly) processing an operation based on the statistics and state of the MqttClientConnection assigned to
         * the MqttOperationQueue. The default is every second.
         ...
         * @return std::shared_ptr<Aws::Crt::Mqtt::MqttConnection> The MqttClientConnection that will be used by the
         * MqttOperationQueue.
         */
         ...
         * @return uint32_t The interval, in milliseconds, that the MqttOperationQueue will wait before checking the
         * queue.
         */
