...
namespace Iotjobs
{
    ...
    void JobExecutionsChangedEvent::LoadFromObject(JobExecutionsChangedEvent& val, const Aws::Crt::JsonView &doc)
    {
        ...
        if (doc.ValueExists("jobs"))
        {
            ...
            val.Jobs = Aws::Crt::Map<Aws::Iotjobs::JobStatus, Aws::Crt::Vector<Aws::Iotjobs::JobExecutionSummary>>();
            for (auto& jobsMapMember : jobsMap.GetAllObjects())
            {
                ...
                val.Jobs->emplace(JobStatusMarshaller::FromString(jobsMapMember.first), std::move(jobsMapValMember));
            }
