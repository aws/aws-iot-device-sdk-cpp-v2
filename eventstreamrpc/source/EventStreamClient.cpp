...
namespace Aws
{
    namespace Eventstreamrpc
    {
        ...
        MessageAmendment::MessageAmendment(const Crt::ByteBuf& payload)
        ...
        MessageAmendment::MessageAmendment(const Crt::List<EventStreamHeader>& headers, Crt::Optional<Crt::ByteBuf>& payload)
        ...
        void MessageAmendment::SetPayload(const Crt::Optional<Crt::ByteBuf> & payload)
        noexcept
        {
            m_payload = payload;
        }
        ...
        ) noexcept
        {
            ...
            if (auto connectionPtr = connection.lock())
            {
                if(aws_event_stream_headers_list_init(&headersArray, connectionPtr->m_allocator)) {
                    ...
                    auto *callbackData = Crt::New<ProtocolMessageCallbackData>(connectionPtr->m_allocator, connectionPtr->m_allocator);
        ...
        EventStreamHeader::EventStreamHeader(const Crt::String& name, const Crt::String& value, Crt::Allocator *allocator)
        ...
        )
        {
            ...
            if(!errorCode)
            {
                ...
                if (connectionObj)
                {
                    connectionObj->m_defaultConnectHeaders.push_back(EventStreamHeader(Crt::String(":version"), Crt::String("0.1.0"), callbackData->allocator));
        ...
        )
        {
            ...
            switch(messageArgs->message_type) {
                ...
                case AWS_EVENT_STREAM_RPC_MESSAGE_TYPE_PING:
                    if(callbackData->onPing) {
                        ...
                        else callbackData->onPing(pingHeaders, Crt::Optional<Crt::ByteBuf>());
                    }
                    break;
            ...
            //ByteCursor payload = ByteCursorFromCrt::ByteBuf(*messageArgs->payload);
        }
    }
}
